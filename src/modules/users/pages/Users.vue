      ...
      <v-card-text class="pa-12">
        <v-row class="d-flex justify-center align-center mb-6">
          ...
          <v-col cols="4" class="d-flex justify-end">
            <v-btn class="justify-end mr-2" color="primary" variant="" prepend-icon="mdi-arrow-left-thin"
              @click="toUserPage()">
              Volver</v-btn>
            <v-btn class="justify-end mr-2" color="primary" variant="tonal" prepend-icon="mdi-account-arrow-up"
              @click="toAsignUser()">
              Asignar Usuario</v-btn>
            <v-btn class="justify-end" color="primary" variant="tonal" prepend-icon="mdi-account-plus"
              @click="toCreateUser()">
              Nuevo Usuario</v-btn>
          </v-col>
        </v-row>
        <v-row>
          <v-data-table color="white" v-model="selected" v-model:items-per-page="itemsPerPage" :headers="headers"
            :items="userList" item-value="name" class="elevation-1">
